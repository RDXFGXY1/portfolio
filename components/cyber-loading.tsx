          ...
          <div className="animate-terminal-scroll">
            {Array.from({ length: 20 }).map((_, i) => (
              <div key={i} className="mb-1">
                {i % 3 === 0 ? (
                  <span>
                    <span className="text-[#00ffff]">[SYS]</span> Initializing module_{Math.floor(Math.random() * 1000)}
                  </span>
                ) : i % 3 === 1 ? (
                  <span>
                    <span className="text-[#ff00ff]">[NET]</span> Connection established to node_
                    {Math.floor(Math.random() * 100)}
                  </span>
                ) : (
                  <span>
                    <span className="text-green-400">[OK]</span> Component loaded successfully
                  </span>
                )}
              </div>
            ))}
